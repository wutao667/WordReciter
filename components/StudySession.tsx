...
import { speakWord, unlockAudio } from '../services/geminiService';
import { RotateCcw, SkipBack, SkipForward, Eye, EyeOff, X, Headphones, Play, Info } from 'lucide-react';
...
const StudySession: React.FC<StudySessionProps> = ({ list, onFinish }) => {
  ...
  const [isUnlocked, setIsUnlocked] = useState(false); // 新增：音频解锁状态
  ...
  const startSequence = useCallback(async () => {
    if (!shuffledWords[currentIndex] || !isUnlocked) return;
    stopAllAudio();
  ...
  }, [shuffledWords, currentIndex, stopAllAudio, isUnlocked]);

  useEffect(() => {
    if (shuffledWords.length > 0 && isUnlocked) {
      startSequence();
    }
  }, [currentIndex, shuffledWords, startSequence, isUnlocked]);
  ...
  if (!isUnlocked) {
    return (
      <div className="fixed inset-0 z-[70] bg-slate-950 flex flex-col items-center justify-center p-8 text-center">
        ...
        <div className="relative z-10 max-w-sm space-y-8">
          <div className="w-24 h-24 bg-indigo-600 rounded-[2.5rem] mx-auto flex items-center justify-center shadow-2xl shadow-indigo-500/40">
            <Headphones className="w-12 h-12 text-white animate-bounce" />
          </div>
          <div className="space-y-4">
            <h2 className="text-3xl font-black text-white">准备好听写了吗？</h2>
            <p className="text-slate-400 text-sm leading-relaxed">
              移动端浏览器（包括微信）需要您的授权才能自动播放语音。点击下方按钮开始学习。
            </p>
          </div>
          <button 
            onClick={handleStartStudy}
            className="w-full py-6 bg-white text-slate-950 rounded-3xl font-black text-lg flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl"
          >
      ...
      <div className="relative z-10 w-full max-w-3xl flex flex-col h-full justify-between py-10">
        <div className="flex justify-between items-center bg-white/5 backdrop-blur-xl p-6 rounded-[2rem] border border-white/10 shadow-2xl">
          <button onClick={onFinish} className="flex items-center space-x-3 text-slate-400 hover:text-white transition-all group">
            ...
            <span className="font-black text-xs tracking-[0.2em] uppercase">结束</span>
          </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-12">
          <div className={`w-full aspect-[16/10] rounded-[4rem] bg-white/5 backdrop-blur-3xl border border-white/10 flex flex-col items-center justify-center p-12 relative shadow-[0_0_100px_rgba(79,70,229,0.15)] transition-all duration-700 ${isPlaying ? 'scale-[1.02] border-indigo-500/30 shadow-[0_0_120px_rgba(79,70,229,0.25)]' : ''}`}>
            <div className="relative text-center w-full h-48 flex flex-col items-center justify-center">
              {isWordVisible ? (
                <h1 className="text-5xl sm:text-8xl font-black tracking-tighter text-white animate-in fade-in zoom-in-90 duration-500 break-all px-4 drop-shadow-2xl leading-tight">
                ...
                <div className="flex flex-col items-center justify-center space-y-6">
                  ...
                  <p className="text-indigo-300/40 text-[10px] font-black uppercase tracking-[0.4em]">正在朗读...</p>
                </div>
              )}
            </div>
        ...
        <div className="space-y-12">
          <div className="flex items-center justify-center space-x-8">
            <button onClick={handlePrevious} disabled={currentIndex === 0} className="w-16 h-16 sm:w-20 sm:h-20 rounded-3xl bg-white/5 border border-white/10 text-white flex items-center justify-center hover:bg-white/10 disabled:opacity-20 transition-all active:scale-90 shadow-xl">
              <SkipBack className="w-6 h-6 sm:w-8 sm:h-8 fill-current" />
            </button>

            <button onClick={startSequence} className={`w-24 h-24 sm:w-28 sm:h-28 rounded-[2.5rem] flex items-center justify-center transition-all duration-500 shadow-2xl active:scale-90 ${isPlaying ? 'bg-indigo-600 text-white shadow-indigo-500/50' : 'bg-white text-slate-950 hover:scale-110'}`}>
              {isPlaying ? (
            ...
            <button onClick={handleNext} className="w-16 h-16 sm:w-20 sm:h-20 rounded-3xl bg-white/5 border border-white/10 text-white flex items-center justify-center hover:bg-white/10 transition-all active:scale-90 shadow-xl">
