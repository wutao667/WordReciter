        ...
        <div className="flex-1 flex flex-col items-center justify-center py-12">
          <div className={`w-full aspect-[16/10] rounded-[4rem] bg-white/5 backdrop-blur-3xl border border-white/10 flex flex-col items-center justify-center p-12 relative shadow-[0_0_100px_rgba(79,70,229,0.15)] transition-all duration-700 ${isPlaying ? 'scale-[1.02] border-indigo-500/30 shadow-[0_0_120px_rgba(79,70,229,0.25)]' : ''} ${hasError ? 'border-amber-500/40' : ''}`}>
            ...
            <div className="relative text-center w-full h-48 flex flex-col items-center justify-center">
              {hasError ? (
                <div className="flex flex-col items-center space-y-4 text-amber-400">
                  ...
                  <button onClick={handleManualPlay} className="px-8 py-3 bg-amber-500 text-slate-900 rounded-full text-[10px] font-black shadow-lg shadow-amber-500/20 active:scale-95 transition-transform">点击重试</button>
                </div>
              ) : isWordVisible ? (
                ...
                <div className="flex flex-col items-center justify-center space-y-6">
                  ...
                  <p className="text-indigo-300/40 text-[10px] font-black uppercase tracking-[0.4em]">正在播报</p>
                </div>
              )}
            </div>
            ...
            <button
              ...
              className={`mt-12 px-10 py-5 rounded-2xl flex items-center space-x-3 transition-all duration-500 font-black text-xs uppercase tracking-widest select-none active:scale-95 ${isWordVisible ? 'bg-indigo-600 text-white shadow-2xl shadow-indigo-500/40' : 'bg-white/10 text-slate-400 hover:bg-white/20 border border-white/5'}`}
            >
