...
import { testGeminiConnectivity, speakWithGlmTTS, speakWordLocal } from '../services/geminiService';
...
const DebugConsole: React.FC<DebugConsoleProps> = ({ onClose }) => {
  ...
  const runApiTest = async () => {
    ...
    addLog('info', '启动 GLM API 连通性自检...');
    const result = await testGeminiConnectivity();
    setApiStatus(result);
    if (result.success) addLog('success', `API 响应正常 (${result.latency}ms)`);
    else addLog('error', `API 连接异常: ${result.message}`);
    setIsTestingApi(false);
  };

  const testTTS = async (mode: 'local' | 'glm') => {
    setIsTestingTTS(true);
    const testText = "Welcome to Lingo Echo vocabulary reciter.";
    addLog('info', `尝试 [${mode === 'local' ? '本地' : 'GLM 云端'}] 语音合成: "${testText}"`);
    
    try {
      if (mode === 'local') {
        await speakWordLocal(testText);
      } else {
        await speakWithGlmTTS(testText);
      }
              ...
              <div className="space-y-8">
                <div className="p-6 bg-slate-50 rounded-3xl border border-slate-200 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">GLM 核心接口自检</h3>
                  </div>
                  <p className="text-xs text-slate-500 leading-relaxed">此测试将通过发送一个简单的 Ping 请求来验证您的 API Key 是否有效，以及智谱 AI 的 Vision/Chat 模型是否可用。</p>
                  <button 
                    onClick={runApiTest} 
                  ...
                  {apiStatus && (
                    <div className={`p-5 rounded-2xl border-2 flex items-start gap-4 animate-in slide-in-from-top-4 ${apiStatus.success ? 'bg-emerald-50/50 border-emerald-100 text-emerald-800' : 'bg-red-50/50 border-red-100 text-red-800'}`}>
                      ...
                      <div>
                        <p className="font-black text-sm">{apiStatus.success ? 'API 连接正常' : 'API 连接异常'}</p>
                ...
                <div className="p-6 bg-slate-50 rounded-3xl border border-slate-200 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-indigo-50 shadow-sm space-y-4">
                      ...
                      <h4 className="font-black text-xs text-slate-800">智谱云端 (GLM-TTS)</h4>
                      ...
                      >
                        <Zap className="w-4 h-4" /> 运行 GLM 测试
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex gap-3 items-start">
                    ...
                    <div className="text-[10px] text-amber-800 font-bold leading-relaxed">
                      诊断建议：如果在微信中“本地测试”无反应，请务必确保已配置有效的 API Key 并执行“GLM 测试”。GLM-TTS 使用标准 WAV 数据流，是移动端环境下发声的最终保障。
                    </div>
                  </div>
                </div>
              </div>
